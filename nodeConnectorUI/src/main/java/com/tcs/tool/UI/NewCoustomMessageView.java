...
public class NewCoustomMessageView extends JDialog implements Subscriber, ActionListener {
	...
	private final ButtonControlPanel buttonPanel;
	private final JTextArea messageArea;
	private final JButton ok, cancel, close, save, delete;
	private final JLabel lbl_name;
	private final JCheckBox replyExpected, sendWhenConnected;
	...
	public NewCoustomMessageView() {
		...
		this.getContentPane()
				.add(LayoutUtils.arrangeComponantsInRow(
						LayoutUtils.arrangeComponantsInColoumn(new JScrollPane(messageArea)).setExpandPolicy(ControlPanel.BOTH), new JSeparator(),
						LayoutUtils.arrangeComponantsInColoumn(false, LayoutUtils.arrangeComponantsInRow(false, lbl_name, replyExpected),
								LayoutUtils.arrangeComponantsInRow(false, name, sendWhenConnected)).setExpandPolicy(ControlPanel.NONE),
						new JSeparator(), buttonPanel.setExpandPolicy(ControlPanel.HORIZONTAL_FULL)));
		Application.getSubscriptionManager().subscribe(this, UIConstants.CREATE_NEW_REQUEST, UIConstants.SAVE_MESSAGE, UIConstants.NEW_MESSAGE_DIALOG,
				txt_reset, txt_close, UIConstants.SAVE_MESSAGE_SUCCESS, UIConstants.CANCEL_MESSAGE, UIConstants.SAVED_MESSAGE_DELETE_SUCCESS);
		reset();
	}

	public static void main(final String[] args) {
		final NewCoustomMessageView msgView = new NewCoustomMessageView();
	...
	public void actionPerformed(final ActionEvent e) {
		if (e.getSource().equals(ok)) Application.getSubscriptionManager().notifySubscriber(UIConstants.CREATE_NEW_REQUEST, e);
		else if (e.getSource().equals(save)) Application.getSubscriptionManager().notifySubscriber(UIConstants.SAVE_MESSAGE, e);
		else if (e.getSource().equals(close)) Application.getSubscriptionManager().notifySubscriber(txt_close, e);
		else if (e.getSource().equals(cancel)) Application.getSubscriptionManager().notifySubscriber(UIConstants.CANCEL_MESSAGE);
	...
	public void onSubscriptionEvent(final SubscriptionEvent event) {
		...
		case UIConstants.CREATE_NEW_REQUEST:
			final UISavableMessage<String> msg = prepareMessageFromInput();
			if (msg != null) {
		...
		case UIConstants.SAVE_MESSAGE:
			final UISavableMessage<String> saveMessage = prepareMessageFromInput();
			if (saveMessage != null) Application.getSubscriptionManager().notifySubscriber(UIConstants.DO_SAVE_MESSAGE, metadata, saveMessage);
			break;
	...
	private void setData(final Message<String> obj) {
	...
	private UISavableMessage<String> prepareMessageFromInput() {
		if (validateData()) {
			final UISavableMessage<String> msg = new UISavableMessage<String>();
