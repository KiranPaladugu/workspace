...
public class MessageDisplayView extends ControlPanel implements Subscriber {
	...
	private final JTextArea dispaly;
	...
	public MessageDisplayView() {
		...
		Application.getSubscriptionManager().subscribe(this, UIConstants.MESSAGE_DISPLAY, UIConstants.GET_DISPLAYING_MESSAGE_REQUEST);
	}
	...
	public synchronized void onSubscriptionEvent(final SubscriptionEvent event) {
		...
		case UIConstants.GET_DISPLAYING_MESSAGE_REQUEST:
			...
			if (dataToSend.trim().length() > 0) {
				...
				final String endStmt = (String) Application.getApplicationContext().get("endOfCommand");
				if (endStmt != null) {
					int index = -1;
				...
				Application.getSubscriptionManager().notifySubscriber(event.getSource().getClass().getName() + ".RESPONSE", this, dataToSend);
			}
		default:
			break;
		}
