...
public class LayoutUtils {
	...
	public static ControlPanel arrangeComponantsInColoumn(final String title, final int fillType, final boolean expand,
	...
	private static GridBagConstraints getCaluculatedGC(final GridBagConstraints gc, final boolean expand, final LayoutUtils utils,
		...
		} else if (!(comp instanceof JLabel) && !(comp instanceof JButton) && !(comp instanceof ButtonControlPanel) && !(comp instanceof JCheckBox)
				&& expand) {
			gc.weightx = 1;
			gc.weighty = 1;
		} else {
	...
	private static void checkExpansionType(final Component comp, final GridBagConstraints gc) {
		if (comp instanceof ControlPanel) {
			final ControlPanel cpanel = (ControlPanel) comp;
